...
| `internal/hook/` | The seam between the engine loop and `gfx`, `input` and `audio`. Each of those packages keeps its plumbing (frame begin and end, resize, the event feed, the audio pull) on unexported methods, wraps the public value in an unexported driver implementing a `hook` interface, and registers a constructor from `init`. `run.go` builds all three through `hook` and hands the game the values `Game()` returns. |
...
allows sixteen samplers per stage, which is why every shadow map lives
in one atlas image and why a new material texture is not free.
...
- Keep the Windows and Linux layers compiling and vetted
  ...
  on hardware; a feature that cannot be implemented there gets an honest
  no-op with a comment, not a stub that claims to work.
