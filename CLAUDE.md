...
- `renderQueue` calls `q.jitterFrame` before anything else, which fills
  `q.projJ`, `q.viewProjJ` and `q.invViewProjJ` with the sub-pixel
  offset temporal anti-aliasing needs (zero when it is off). Everything
...
- The velocity image holds the object's own motion only, in texture
  ...
  in the three vec4s past `atten`, which no other program declares.
