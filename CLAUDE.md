...
|---|---|
| `engine/engine.go`, `engine/run.go`, `engine/headless.go`, `engine/debug.go`, `engine/flycam.go`, `engine/url.go` | The public runtime package: `Run`, `Config`, `Game`, `Context`, the loop (fixed step or turn-based), the fixed view and letterboxing, the F3 overlay, headless mode, the fly camera. |
...
| `input/` | Physical keys, native keyboard-layout snapshots, per-update `State`, gamepad metadata, action maps. |
...
| `internal/render/` | The Vulkan backend: device, swapchain, frames in flight, the submit goroutine (`submit.go`), images, buffers, pipelines and the pipeline cache file (`pipecache.go`), descriptor sets, targets, readback. |
| `internal/hook/` | The boundary between the engine loop and `gfx`, `input` and `audio`. Each of those packages keeps its plumbing (frame begin and end, resize, the event feed, the audio pull) on unexported methods, wraps the public value in an unexported driver implementing a `hook` interface, and registers a constructor from `init`. `engine/run.go` builds all three through `hook` and hands the game the values `Game()` returns. |
...
`Poll(true)`. Before it polls, `windowFamily.waitFrames` waits for each
window's next frame slot on the GPU (`hook.FrameWaiter`, which the gfx
driver implements with `Renderer.WaitFrame`), so a frame's input is read
after the GPU wait rather than going stale during it; `Begin` waits
itself when that was not called, which keeps a wrapped driver correct.
`TestFrameWaitsBeforePoll` pins the order wait, poll, update, draw. Input
...
- Uploads inside a frame (`NewMesh`, `Mesh.Update`, `UpdateSkinned`,
  ...
  synchronous path for readbacks and bakes. Every submission, the
  batch's included, goes through the submit goroutine described below.
  Everything
...
- The device's queue belongs to a goroutine locked to an OS thread
  (`internal/render/submit.go`). `EndFrame` records the frame's end and
  posts a job to it, which submits and presents while the game goroutine
  goes on to the next frame; MoltenVK encodes the whole Metal frame
  inside `vkQueueSubmit2`, which is what moves off the game goroutine.
  `Device.submit` is the only way work reaches the queue: frames, upload
  batches and `OneShot` all post jobs, which run in order, and
  `TestQueueCallsStayInSubmit` fails if `vk.QueueSubmit2` or
  `vk.QueuePresentKHR` appear anywhere else in the package. Calls Vulkan
  wants externally synchronised with the queue or the swapchain
  (`vkQueueWaitIdle`, `vkDeviceWaitIdle`, `vkAcquireNextImageKHR`,
  rebuilding the swapchain) drain the goroutine and hold `submitter.mu`.
  A fence handed to a job is the goroutine's until the job has run:
  nothing waits on it, resets it or reads its status before that
  (`submitter.await`, `hasRun`, `Device.fenceSignalled`, `waitFence`), or
  the validation layer reports a threading error. A failed job is kept
  and every later job is skipped, and the next wait returns the error,
  which is how a lost device reaches the loop. The goroutine calls
  purego directly with its own argument slice, because the vk package's
  allocation-free wrappers share one slice that only the recording
  goroutine may use; the same goes for any other goroutine that calls
  Vulkan, such as the pipeline workers, which use the plain wrappers.
- Pipelines are built through one `VkPipelineCache` per device, created
  internally synchronised so several goroutines build through it at
  once, and share one `VkShaderModule` per SPIR-V program
  (`internal/render/pipecache.go`). `newGraphics` opens the cache from
  a file under `Config.PipelineCacheDir` named for the device, driver
  and `engineShaderKey` (a hash of every program in `gfx/shaders`;
  `TestEngineShadersComplete` checks the list), and the device writes it
  back atomically every few seconds while pipelines are being built
  (`SavePipelineCacheSoon`, called from `end`) and when it is destroyed.
  The engine points the directory at `os.UserCacheDir()/bunyip` unless
  `Config.NoPipelineCache`; tests and benchmarks leave it empty, which
  keeps the cache in memory. `newGraphics` builds the four 2D pipelines
  in place and collects everything `initMeshPass`, `initPost`,
  `initReflections` and `initLines` build into a `render.PipelineBatch`
  that compiles on worker goroutines; `waitPipelines` waits for it, and
  `renderQueue` (for a 3D or post frame), `renderScene` and `destroy`
  call it before recording with any of those pipelines. A pipeline built
  on a worker has its `Layout` at once and its `Handle` only after
  `Wait`, so `Shader.pipeline` and `pipeCache.at` wait for the variant
  they return. `startSceneVariants` (after `prepareDraws`) and
  `startSampleVariants` (when `PostSettings.Samples` changes) start the
  variants a frame is about to need side by side, so a new material or
  sample count waits for the slowest rather than the sum.
- `Mesh.Update` and `UpdateSkinned` write into buffers an earlier update
  replaced (`gfx/geometry_pool.go`): a replaced buffer waits on its frame
  slot's list until the slot's fence has been waited on, then joins the
  pool, and an update takes the smallest pooled buffer that fits. A mesh
  updated every frame settles at a few buffers and allocates none;
  `TestMeshUpdateReusesBuffers` counts them. Packing reuses one scratch
  slice. A mesh keeps geometry in main memory as `meshGeometry` says:
  the slices it was given (`KeepVertices`, what `NewMesh` does), a
  compact copy of positions and two- or four-byte indices
  (`KeepPositions`, what a `Model`, a skinned mesh and a terrain's chunks
  keep), or nothing (`KeepNone`, `Mesh.ReleaseGeometry`). `Intersect` and
  occlusion read it, and `TestIntersectKeptGeometry` checks every form
  hits exactly what the full vertices did.
...
- The console is built in `runOnce` before `Init`, because it tees the
  ...
  game and debug overlay. It cannot import `engine` without a cycle,
  so it declares `Frame` and the one-method `Host` that
...
molten-vk` on macOS). `go get github.com/matjam/bunyip/engine`. Build with
`CGO_ENABLED=0`; there is nothing to link against.

## The shape of a game

Import `github.com/matjam/bunyip/engine` for the runtime. The repository root
contains module metadata and project files; public Go packages live in subdirectories.
...
func main() {
	engine.Run(engine.Config{Title: "Game", Width: 1280, Height: 720}, &game{})
}
```
...
| Need | Look at |
|---|---|
| The loop, views, windows, screenshots, headless runs | `engine.Config`, `engine.Context`, `Context.NewWindow`, `engine.Window` |
| Native host embedding with Run-managed event dispatch | `Config.Parent`, `NativeParent`, `Context.SetBounds` |
...
| A debug console and panels over a running game | `console`, `engine.Config.Console` |
