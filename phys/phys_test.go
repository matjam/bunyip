...
func TestFallAndRest2D(t *testing.T) {
	...
	run(w, 3)
	bt, _ := w.Get[gfx.Transform2](ball)
	bb, _ := w.Get[Body2](ball)
...
func TestStackTriggerRaycast2D(t *testing.T) {
	w := ecs.NewWorld()
	w.SetResource(Settings2{Gravity: lin.V2(0, -10), Substeps: 8, Iterations: 12})
	w.AddSystem("phys", System2)
	...
	trig := false
	for _, ev := range w.Events[Trigger2]() {
	...
	vt, _ := w.Get[gfx.Transform2](visitor)
...
func TestLayersAndKinematic2D(t *testing.T) {
	...
	run(w, 1)
	gt, _ := w.Get[gfx.Transform2](ghost)
	if gt.Position.Y > 0 {
	...
	run(w, 2)
	pt, _ := w.Get[gfx.Transform2](plat)
	rt, _ := w.Get[gfx.Transform2](rider)
...
func TestFallRestAndStack3D(t *testing.T) {
	w := ecs.NewWorld()
	w.SetResource(Settings3{Gravity: lin.V3(0, -10, 0), Substeps: 8, Iterations: 12})
	...
	run(w, 4)
	bt, _ := w.Get[gfx.Transform](ball)
	bb, _ := w.Get[Body3](ball)
	if math.Abs(float64(bt.Position.Y-1)) > 0.05 || bb.Vel.Len() > 0.05 {
		t.Fatalf("ball should rest at y=1: %v vel %v", bt.Position, bb.Vel)
	}
	for i, b := range boxes {
		xt, _ := w.Get[gfx.Transform](b)
	...
	tt, _ := w.Get[gfx.Transform](tilted)
...
func TestBounceRollRaycast3D(t *testing.T) {
	...
	run(w, 0.5)
	found := false
	for _, ev := range w.Events[Trigger3]() {
