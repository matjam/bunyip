...
func TestHingeMotor3D(t *testing.T) {
	...
	j, _ := ecs.Get[HingeJoint3](w, hinge)
...
func TestHingeLimit3D(t *testing.T) {
	...
	bt, _ := ecs.Get[gfx.Transform](w, bar)
...
func TestRevoluteMotorAndLimit2D(t *testing.T) {
	...
	j, _ := ecs.Get[RevoluteJoint2](w, pin)
	lowest := float32(0)
	for range 240 {
		w.Update(step)
		lowest = min(lowest, j.Angle(w))
...
func TestBallJoint3D(t *testing.T) {
	...
	lt, _ := ecs.Get[gfx.Transform](w, limb)
...
func TestRagdoll3D(t *testing.T) {
	...
	head, _ := ecs.Get[gfx.Transform](w, r.Parts[RagdollHead])
	...
	check := func() {
		for name, je := range r.Joints {
			...
			ta, _ := ecs.Get[gfx.Transform](w, a)
			tb, _ := ecs.Get[gfx.Transform](w, b)
	...
	for _, name := range RagdollParts {
		tr, _ := ecs.Get[gfx.Transform](w, r.Parts[name])
		b, _ := ecs.Get[Body3](w, r.Parts[name])
		bone := r.Bones[name]
	...
	r.Pose(w, pos, rot)
	pt, _ := ecs.Get[gfx.Transform](w, r.Parts[RagdollPelvis])
	pb, _ := ecs.Get[Body3](w, r.Parts[RagdollPelvis])
	...
	r.Despawn(w)
	if _, ok := ecs.Get[gfx.Transform](w, r.Parts[RagdollHead]); ok {
...
func TestDynamicCCD(t *testing.T) {
	for _, ccd := range []bool{true, false} {
		...
		run(w2, 0.5)
		t2a, _ := ecs.Get[gfx.Transform2](w2, a2)
		t2b, _ := ecs.Get[gfx.Transform2](w2, b2)
