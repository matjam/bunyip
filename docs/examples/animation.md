...
title: Animation
example: animation
summary: keyframe clips on 2D sprites and 3D transforms, a flipbook, crossfades driven by Finished events, and three skeletal arms showing an animation event, two-bone IK and a blend space
---

This program shows both halves of animation in one scene. Keyframe clips
from [anim](../pkg/anim.html) drive plain components on the
[entity component system](../pkg/ecs.html): sprite positions, sizes,
...
[gfx.AnimPlayer](../pkg/gfx.html#AnimPlayer) over a model's node
...
[gltf.Load](../pkg/gltf.html#Load) returns from a file.
...
[The animation guide](../guides/animation.html) covers both.
...
it with a function from [tween](../pkg/tween.html). Rotations are in
