...
The flags are `-seconds N`, `-shot file.png` and `-dir path`, which is
the asset directory; it defaults to a directory under the system
temporary directory and is created and seeded when empty. P packs the
directory and Escape quits.
