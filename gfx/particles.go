...
type particlePass struct {
	flat  [blendCount]*render.Pipeline // 2D, into the sprite stream's pass
	scene [blendCount]*render.Pipeline // 3D, over the finished scene
	push  push2D
...
// flatPipeline returns the 2D particle pipeline for a blend mode,
// building it on first use. It writes the same formats as the sprite
// pipelines, because it records into the same pass.
func (g *Graphics) flatPipeline(b Blend) (*render.Pipeline, error) {
	if p := g.particles.flat[b]; p != nil {
		return p, nil
	}
	bindings, attrs := particleLayout()
	p, err := g.r.Device.NewPipeline(render.PipelineDesc{
	...
	g.particles.flat[b] = p
	return p, nil
}

// scenePipeline returns the 3D particle pipeline for a blend mode. It
// has no depth attachment: the pass it records into leaves the depth
// image readable, and the fragment program tests against it by hand.
func (g *Graphics) scenePipeline(b Blend) (*render.Pipeline, error) {
	if p := g.particles.scene[b]; p != nil {
		return p, nil
	}
	bindings, attrs := particleLayout()
	p, err := g.r.Device.NewPipeline(render.PipelineDesc{
	...
	g.particles.scene[b] = p
	return p, nil
}

func (p *particlePass) destroy() {
	for _, set := range [][blendCount]*render.Pipeline{p.flat, p.scene} {
		for _, pipe := range set {
			if pipe != nil {
				pipe.Destroy()
			}
		}
	}
	p.flat, p.scene = [blendCount]*render.Pipeline{}, [blendCount]*render.Pipeline{}
}
...
func (g *Graphics) drawFlatParticles(cb vk.VkCommandBuffer, q *drawQueue, at int, d draw2D, all bool, vp vk.VkRect2D) (int, error) {
	...
	for first := true; at < len(batches); first = false {
		...
		pipe, err := g.flatPipeline(b.blend)
		if err != nil {
			return at, err
		}
		pp.off = 0
...
func (g *Graphics) drawSceneParticles(cb vk.VkCommandBuffer, q *drawQueue, t *sceneTargets, aspect float32) error {
	...
	for _, b := range q.parts.scene {
		pipe, err := g.scenePipeline(b.blend)
		if err != nil {
			return err
		}
		pp.off = 0
