...
// instance stream. It returns three groups in the order they are
// recorded: the opaque draws, the blended draws the order-independent
// pass accumulates, and the blended draws that still draw in sorted
// order. Culled draws sort to the end of their group and stay in the
// lists for the shadow pass, which sees them from the light;
// q.visOpaque, q.visOIT and q.visBlended count the draws the camera sees
// at the front of each. It runs before writeUniforms and leaves each
// draw's bounding sphere on the draw, for the shadow pass to cull
// against, and the furthest caster's reach against the light in
